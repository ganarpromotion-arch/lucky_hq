...
async def make_video_for_archived_job(job_id: int, seed: int | None = None,
                                       preset_image_path: str | None = None) -> None:
    """보관된 곡 1개를 영상으로 만들고 텔레그램으로 보낸다.
    배치와 무관하게 사이트에서 사용자가 체크한 곡 처리용. 자체 DB 세션 사용.

    preset_image_path가 있으면 그 이미지(시안 선택 결과)를 영상에 사용,
    없고 seed만 있으면 PIL 시드로 재현."""
    db = SessionLocal()
    try:
        ...
        await _make_and_send_video(db, batch=None, job=job, seed=seed,
                                    preset_image_path=preset_image_path)
        db.commit()
    except Exception as e:
